...
RGB retrieve_from_pointer_struct(RGB *x) {
    ...
    \______   \__| ____ |  |___/  |_|__| ____        _________\_____   \
    |       _/  |/ ___\|  |  \   __\  |/ ___\      /  ___/  _ \ /   __/
    |    |   \  \  \___|   Y  \  | |  / /_/  >     \___ (  <_> )   |   
    |____|_  /__|\___  >___|  /__| |__\___  /     /____  >____/|___|   
           \/        \/     \/       /_____/           \/      <___>   
...
void set_through_pointer_struct1(RGB *x) {
    ...
    RGB colorFlieder = {colorFlieder.r = 0.86, colorFlieder.g = 0.82, colorFlieder.b = 1.0};
...
int32_t frumpleflabblefizzledipp(int32_t x) {
    ...
    int32_t flabbleZWS = flabble(&zwsX, &x);

    int32_t frumpleX;

    int32_t y = 27;

    frumple(flabbleZWS, &y, &frumpleX);
...
bool ends_with_fruit(PileOfPancakes p) {
    ...
    while(p.further_layers != NULL){            // solange noch nicht bei der letzten Schicht angekommen wurde
        p = *p.further_layers;                  // wird zur nächsten schicht übergegangen
    }
    
    if (p.layer == Fruit)                       // da an diesem Punkt im Programm die letzte schicht erreicht wurde
    {
        return true;                            // wird zurückgegeben ob sie eine Fruchtschicht ist oder nicht
    }
    else
    {
        return false;
...
int32_t count_fruit_layers(PileOfPancakes p) {
    ...
    int32_t fruitlayers = 0;                    // Zählvariable "trackt" Fruchtschichten                     
   
    while(p.further_layers != NULL)             // solange noch nicht bei der letzten Schicht angekommen wurde
    {
        if(p.layer == Fruit){                   // prüfe ob die Aktuelle Schicht eine Fruchtschicht ist
            fruitlayers++;                      // wenn das der Fall ist wird der "Fruchtschichttracker" inkrementiert
        }

        p = *p.further_layers;                  // jetzt wird wieder zur nächsten schicht übergegangen
    }

    if(p.layer == Fruit)                        // da die abbruchbedingung der while-Schleife bei nichtexistierender nächster schicht eintrifft
    {
        fruitlayers++;                          // wird für die letzte schicht, die von der schleife nicht erreicht wird, nochmal geprüft 
    }

    return fruitlayers;                         // Rückgabe der Fruchtschichten
}
